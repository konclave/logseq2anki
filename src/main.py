...
from src.parser import scan_journals, LogseqNote
...
def run():
    console.print(Panel.fit("[bold blue]Logseq to Anki Sync[/bold blue]", subtitle="German Edition"))
    
    journals_dir = os.path.expanduser("~/Sync/logseq/journals/")
    deck_name = "Logseq::German"
    ...
    if not llm.api_key:
        console.print("[yellow]Warning: GEMINI_API_KEY not found in environment. Example generation will be skipped.[/yellow]")

    # 3. Check Anki
    try:
        anki.invoke("version")
    except Exception:
        console.print("[red]Error: Cannot connect to Anki. Make sure Anki is running and AnkiConnect is installed.[/red]")
        return

    # Ensure deck exists
    default_deck_name = "Logseq::German"
    ...
    with Progress() as progress:
        ...
        for note in notes:
            ...
            if extra_tags:
                ...
                target_deck = extra_tags[0].capitalize()
            else:
            ...
            # Check if exists
            # We use a combined front and translation for better duplicate checking if needed
            if anki.check_if_exists(note.front, target_deck):
                skipped_notes += 1
            ...
            fields = {
                "Text": f"{note.front} {{{{c1::{note.cloze}}}}}" + (f"<br><br><i>{example_text}</i>" if example_text else ""),
                "Back Extra": ""
            }
            ...
            try:
                ...
                table.add_row(note.front, target_deck, "[green]Added[/green]", display_example)
